  ...
  try {
    ...
    if (examCheck.rowCount === 0) {
      return res.status(HTTP_STATUS.NOT_FOUND).json({
        success: false,
        error: { code: 'EXAM_NOT_FOUND', message: 'Exam record not found.' },
      });
    }

    const currentExam = examCheck.rows[0];

    // RBAC: If faculty, ensure they manage the course
    if (req.user.role === ROLES.FACULTY) {
      const courseCheck = await query('SELECT faculty_id FROM courses WHERE id = $1', [currentExam.course_id]);
      if (courseCheck.rowCount === 0 || courseCheck.rows[0].faculty_id !== req.user.facultyId) {
        return res.status(HTTP_STATUS.FORBIDDEN).json({
          success: false,
          error: { code: 'FORBIDDEN_EXAM_ACCESS', message: 'You can only update exams for your assigned courses.' },
        });
      }
    }
    ...
    await query(`UPDATE exams SET ${updateFields.join(', ')} WHERE id = $${pIdx}`, updateParams);
